...
def load_features(clip_name: str, of_params: OfParams) -> np.ndarray:

    dir_name = get_feature_dir_name_new(of_params)

    # try:
    #     feature_array = load_feature_from_file(clip_name, dir_name)
    # except FileNotFoundError:
    _, left_images, right_images = self._get_frames(convert_to_gray=True)
    feature_array = calculate_optical_flow(of_params, left_images, right_images)
    save_feature_to_file(feature_array, clip_name, dir_name)

    feature_arrays[clip_name] = feature_array
